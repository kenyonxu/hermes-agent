...
class SessionStore:
    ...
    def _open_session_db_for_active_scope(self):
        ...
        """
        from hermes_state import DEFAULT_DB_PATH, SessionDB, _default_db_path

        path = Path(_default_db_path())
        def _open():
            # Local fork (7/23 deadlock fix): when the active scope resolves
            # to the frozen default path (the root home of this single-home
            # deployment), serve the process-wide shared writer singleton so
            # gateway writes share one SQLite connection (no WAL write-lock
            # contention between gateway / cron / session store). Multiplexed
            # secondary profiles keep upstream's per-profile handles (#88532),
            # and tests (tmp HERMES_HOME != frozen default) never hit this.
            if path == DEFAULT_DB_PATH:
                from hermes_state import get_shared_session_db

                shared = get_shared_session_db(path)
