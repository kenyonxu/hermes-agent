...
class SessionStore:
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig,
        ...
        # Local fork (7/23 deadlock fix): pin the process-wide shared writer
        # singleton so every gateway write path shares one SQLite connection
        # (avoids WAL write-lock contention). Upstream #88532's per-profile
        # handle cache remains as the unpinned fallback (tests / multiplexed
        # roots) — pinned reads never reach it.
        self._db = None
        try:
