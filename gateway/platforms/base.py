...
class BasePlatformAdapter(ABC):
    ...
    async def _send_with_retry(
        ...
        async def _send(text: str) -> "SendResult":
            # Fork graft (478805be05): every attempt — initial, retries, the
            # delivery-failure notice, and the plain-text fallback — is bounded
            # by _send_with_deadline so a black-holed transport can never freeze
            # the send path (see test_send_deadline.py).
            return await self._send_with_deadline(chat_id=chat_id, content=text, reply_to=reply_to, metadata=metadata)
