...
class BasePlatformAdapter(ABC):
    ...
    async def _send_with_deadline(
        ...
        *,
        ...
        """
        return await self._await_send_deadline(
            self.send(chat_id=chat_id, content=content, reply_to=reply_to, metadata=metadata),
            chat_id=chat_id,
        )

    async def _await_send_deadline(self, coro, *, chat_id: str) -> "SendResult":
        """Await a send-shaped coroutine bounded by ``_send_deadline_seconds`` (same
        timeout→non-retryable mapping as ``_send_with_deadline``; exists so upstream's
        overridable send helpers, e.g. ``_send_plain_fallback``, stay deadline-bounded
        without duplicating their content at the call site)."""
    ...
    async def _send_with_retry(
        ...
        async def _send(text: str) -> "SendResult":
            # Send-deadline governance (ours, kept): every wire send in this loop — first
            # attempt, retries and the delivery-failure notice — goes through
            # _send_with_deadline so a black-holed platform request cannot pin the turn.
        ...
        fallback_result = await self._await_send_deadline(
            self._send_plain_fallback(chat_id, content, reply_to=reply_to, metadata=metadata),
            chat_id=chat_id,
        )
