...
def _housekeeping_cron_session_prune() -> None:
    """Prune cron sessions older than 3 days (fork 24bfe7840b, grafted onto the
    upstream housekeeping chores list after the god-file split).

    Cron jobs create a session every tick, filling state.db with small sessions
    that bloat FTS indexes and increase write-lock hold times. Unconditional
    (not config-gated): this replaced the old always-on tick in the fork."""
    from hermes_state_registry import acquire, release_or_close
    _cdb = acquire()
    try:
        _pruned = _cdb.prune_sessions(older_than_days=3, source="cron")
        if _pruned > 0:
            logger.info("Cron session prune: removed %d old session(s)", _pruned)
    except Exception as e:
        logger.debug("Cron session prune error: %s", e)
    finally:
        release_or_close(_cdb)
...
def _start_gateway_housekeeping(
    ...
    chores += [
        ...
        (120, "Cron session prune (3d+)", _housekeeping_cron_session_prune)]
