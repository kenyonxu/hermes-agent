...
def run_job(
    ...
    *,
    ...
    # ---------------------------------------------------------------------
    # Local fork (7/23 deadlock fix): use the scheduler-wide shared SessionDB
    # instance — avoids per-tick _connect_and_init DDL checks that cause
    # SQLite write-lock contention. The singleton is initialized with a
    # bounded timeout (see hermes_state.get_shared_session_db) so a wedged
    # sqlite3.connect does not block every cron tick — the worker thread is
    # abandoned and subsequent calls return None (matching the pre-singleton
    # fallback behavior). #72782 late-result close semantics are absorbed
    # inside get_shared_session_db.
